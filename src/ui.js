...
dust.loadSource(dust.compile("<table class='potential'><tr class='head'><td>Rank<th>Name<th>Position<th>Points<th>VORP<th>ID{#players}<tr data-id='{id}' data-pos='{fantasy_position}'><td>{rank}<td class='n'>{first_name} {last_name}<td>{fantasy_position} ({pos_rank})<td>{points}<td>{vorp}<td>{id}</tr>{/players}</table>",
                            "potentials"));

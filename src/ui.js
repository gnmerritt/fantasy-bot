...
dust.loadSource(dust.compile("<table class='potential'><tr class='head'><td>Rank<th>Name<th>Team<th>Position<th>Points<th>VORP<th>ID{#players}<tr data-id='{id}' data-pos='{fantasy_position}'><td>{$idx}<td class='n'>{first_name} {last_name}<td>{team}<td>{pos} ({pos_rank})<td>{points}<td>{vorp}<td>{id}</tr>{/players}</table>",
                            "potentials"));
